    ...
    for i, idx in tqdm(enumerate(idxs)):
        ...
        if test and add_frame_to_list:
            ...
            if test_dataset.all_fwd_flow is not None:
                ...
                fwd_flow = cv2.resize(test_dataset.all_fwd_flow[test_dataset.all_fbases[fbase]].cpu().numpy(), (W, H), interpolation=cv2.INTER_NEAREST) 
                fwd_mask = cv2.resize(test_dataset.all_fwd_mask[test_dataset.all_fbases[fbase]].cpu().numpy(), (W, H), interpolation=cv2.INTER_NEAREST)
                bwd_flow = cv2.resize(test_dataset.all_bwd_flow[test_dataset.all_fbases[fbase]].cpu().numpy(), (W, H), interpolation=cv2.INTER_NEAREST)
                bwd_mask = cv2.resize(test_dataset.all_bwd_mask[test_dataset.all_fbases[fbase]].cpu().numpy(), (W, H), interpolation=cv2.INTER_NEAREST)
            ...
            if test_dataset.all_invdepths is not None:
                invdepths = torch.from_numpy(cv2.resize(test_dataset.all_invdepths[test_dataset.all_fbases[fbase]].cpu().numpy(), (W, H), interpolation=cv2.INTER_NEAREST)).to(args.device)
        ...
        if test:
            fbase = train_dataset.get_frame_fbase(idx)
            gt_rgb = test_dataset.all_rgbs[test_dataset.all_fbases[fbase]].cpu().numpy()
